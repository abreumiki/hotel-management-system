...
admin_buttons = ["btnHome1", "btnRoom1", "btnHistory1", "btnReport1", "btnEmployee1", "btnSettings1", "btnHome2", "btnRoom2", "btnHistory2", "btnReport2", "btnEmployee2", "btnSettings2"]
...
class Hotel(QtWidgets.QMainWindow):
    ...
    def red_mark(self, list):
        for widget_name in list:
            ...
            is_line_edit = isinstance(widget, QtWidgets.QLineEdit); is_combo_box = isinstance(widget, QtWidgets.QComboBox); is_spin_box = isinstance(widget, QtWidgets.QSpinBox); is_text_edit = isinstance(widget, QtWidgets.QTextEdit)
...
class Cashier(Admin):
    def __init__(self):
        ...
        for name in cashier_buttons:
            button = getattr(self, name)
            button.clicked.connect(lambda checked, button_name=name: self.selected_menu(button_name))
        self.btnRFIDenter.hide(); self.btnRFIDout.hide(); self.show_promos()
        ...
        self.inputs_personal = ['txtCfirstname', 'txtClastname', 'cboCidtype', 'txtCidnum', 'txtCcontactno']; self.inputs_room = ['cboCroomtype', 'cboCroomno', 'txtroomrate', 'txtCrfidNo']
    ...
    def proceed_to_payment(self):
        self.stackedWidget.setCurrentIndex(4); self.lbldatetoday.setText(self.lblDate.text())
        ...
        self.txtOpromo.show(); self.lblpromo.setText('Promo: '+self.txtOpromo.text()); self.txtOpromo.hide(); self.lblstay.setText('Length of stay: '+self.txtOstay.text()); self.lblrecep.setText('Receptionist: '+self.lblFname.text()+' '+self.list[3])
        self.clear_fields(self.check_out_fields)
        self.btnProceedPaymanet.setEnabled(False); self.btnCoutCancel.setEnabled(False)
    ...
    def rfid_checkout(self):
        try:
            ...
            connection = pymysql.connect(host="localhost", port=3308, user="miki", password="miki", database="hotel")
            cursor = connection.cursor()
            command = """SELECT client_id, first_name, last_name, room_no, checkin_date, checkin_time, promo FROM clients WHERE rfid_no = %s"""
    ...
    def compute_length_of_stay(self):
        ...
        checkin_datetime = datetime.strptime(self.checkin_datetime, datetime_format); checkout_datetime = datetime.strptime(self.checkout_datetime, datetime_format)
        time_diff = checkout_datetime - checkin_datetime

        days_passed = time_diff.days
        total_seconds_passed = time_diff.total_seconds(); total_hours_passed = int(total_seconds_passed // 3600); total_minutes_passed = int((total_seconds_passed % 3600) // 60)
        if days_passed == 0:
            ...
            self.total_length_stay = f"{days_passed}d"
    ...
    def edit_client_info(self):
        try:
            ...
            connection = pymysql.connect(host="localhost", port=3308, user="miki", password="miki", database="hotel")
            cursor = connection.cursor()
            command = """SELECT client_id, first_name, last_name, contact_no, id_type, id_number, room_no, room_type, room_rate rfid_no, promo FROM clients WHERE client_id = %s"""
            cursor.execute(command, data_value)
            records = cursor.fetchall()
            for data in records:
                data_list.extend(data)
            self.txtCid.setText('HMS-23-0'+data_list[0]); self.txtCfirstname.setText(data_list[1]); self.txtClastname.setText(data_list[2]); self.txtCcontactno.setText(data_list[3]); self.cboCidtype.setCurrentText(data_list[4]); self.txtCidnum.setText(data_list[5])
            self.cboCroomno.setCurrentText(data_list[6]); self.cboCroomtype.setCurrentText(data_list[7]); self.txtroomrate.setText(data_list[8]); self.txtCrfidNo.setText(data_list[9]); self.cboCpromos.setCurrentText(data_list[10])
        except:
    ...
    def add_client_back(self):
        ...
        self.SWClient.setCurrentIndex(0); self.show_clientinfo_table()
        #clear fields ======================================================

    def add_client_next(self):
        self.c_personal_input = [self.txtCid.text(), self.txtCfirstname.text().replace(" ", ""), self.txtClastname.text().replace(" ", ""), self.cboCidtype.currentText(), self.txtCidnum.text().replace(" ", ""), self.txtCcontactno.text().replace(' ','')]
        if '' in self.c_personal_input:
            self.showDialog('Please fill out all the fields!'); self.red_mark(self.inputs_personal)
        else:
            self.lblC2.setText(self.lblClientPage.text()); self.back_to_normal(self.inputs_personal)
            self.SWClient.setCurrentIndex(2)
    ...
    def add_client_proceed(self):
        promo_holder = ''
        self.c_room_input = [self.cboCroomtype.currentText(), self.cboCroomno.currentText(), self.txtCrfidNo.text().replace(" ", "")]
        if '' in self.c_room_input:
            ...
            if self.cboCroomno.currentText() == 'No available room right now':
                ...
                self.c_room_input.insert(2, self.cboCpromos.currentText())
                if self.cboCpromos.currentText() == 'No Available Promo':
                    promo_holder=''
                else:
                    promo_holder=self.cboCpromos.currentText()
                    self.c_room_input = [self.cboCroomtype.currentText(), self.cboCroomno.currentText(), self.txtroomrate.text(), promo_holder,self.txtCrfidNo.text().replace(" ", "")]
                ...
                print(fields)
                for f in range(len(self.all_info)):
                    field_name = fields[f]
    ...
    def client_added(self):
        self.SWClient.setCurrentIndex(0)
        self.widget.close()
        print(self.all_info)
        ...
        if self.lblClientPage.text() == 'Add New Client':
            ...
            query = """UPDATE clients SET client_id = %s, first_name = %s, last_name = %s, contact_no = %s, id_type = %s, id_number = %s, room_type = %s, room_no = %s, promo = %s, rfid_no = %s, status = %s WHERE client_id = %s;"""
            self.data_value = (self.all_info[0].replace('HMS-23-0', ''), self.all_info[1], self.all_info[2], self.all_info[5], self.all_info[3], self.all_info[4], self.all_info[6], self.all_info[7].replace('RM-0', ''), self.all_info[8], self.all_info[9], 'New', self.all_info[0].replace('HMS-23-0', ''))
            text='Changes Saved!'
        self.exec_query(query, self.data_value)
        QTimer.singleShot(300, lambda: self.showDialog(text))
